...
Os algoritmos foram desenvolvidos em C, utilizando a leitura de arquivos para a entrada dos números a serem ordenados (dados). Os experimentos foram executados no mesmo ambiente computacional, garantindo igualdade nas condições de teste.
...
|`<time.h>`                | Medir o tempo de execução dos algoritmos|
...
Assim, é possível observar o comportamento dos algoritmos em casos médios, piores casos e entradas favoráveis.
...
Para a validação do experimento, cada teste foi executado três vezes para cada entrada. Ao final, para a análise dos resultados, foi calculada a média dos tempos obtidos, reduzindo possíveis variações externas.
...
- __Entrada ordenada__: No conjunto de dados ordenados, o Smoothsort apresentou um desempenho extremamente superior aos demais, executando em aproximadamente **0,042s**. Isso ocorre pois esse método de ordenação possui um algoritmo adaptativo, conseguindo aproveitar entradas ordenadas ou parcialmente ordenadas para reduzir significativamente seu custo computacional.

## Análise assintótica 
A análise assintótica tem como objeto avaliar o comportamento dos algoritmos à medida que o tamanho da entrada cresce. Permitindo, assim, a compreensão teórica do desempenho dos métodos de ordenação implementados e a comparação desses resultados com os dados obtidos experimentalmente.
...
Portanto, a complexidade assintótica é dada por:
$$O(n \cdot \log n)$$.
...
### Heap D-ário (D = 3)
  ...
  | Ajuste da heap        | $O(\log_3 n)$        | A altura reduzida da heap diminui a quantidade de níveis percorridos durante as operações de reorganização. |
...
### Smoothsort
  | Operação                          | Complexidade          | Explicação |
  |-----------------------------------|-----------------------| -----------|
  | Construção das heaps de Leonardo.  | $O(n)$                |O algoritmo constrói estruturas especiais chamadas heaps de Leonardo. |
...
Ademais, a complexidade assintótica é dada por:
  | Caso             | Complexidade          | Explicação |
  |------------------|-----------------------| -----------|
  | Melhor caso      | $O(n)$                | Quando os dados já estão ordenados, o algoritmo aproveita a organização prévia e reduz significativamente o número de operações. |
  | Pior caso        | $O(n \cdot \log n)$   | Em entradas desorganizadas, o comportamento assintótico aproxima-se do HeapSort tradicional. |
...
- A distribuição dos dados influencia diretamente na eficiência dos algoritmos.
